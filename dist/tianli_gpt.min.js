...
(function(p){typeof define=="function"&&define.amd?define(p):p()})(function(){"use strict";const p='<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><g id="机器人" fill="none" fill-rule="evenodd" stroke="none" stroke-width="1"><path id="形状结合" fill="#444" fill-rule="nonzero" d="M34.718 5.036a2.59 2.59 0 0 1 2.127 2.977l-1.05 6.302h2.58A8.625 8.625 0 0 1 47 22.94v11.5a8.625 8.625 0 0 1-8.625 8.625H9.625A8.625 8.625 0 0 1 1 34.44v-11.5a8.625 8.625 0 0 1 8.625-8.625h2.58l-1.05-6.302a2.588 2.588 0 0 1 5.105-.85l1.15 6.9q.021.127.03.252h13.12q.009-.126.03-.253l1.15-6.9a2.59 2.59 0 0 1 2.978-2.126m3.657 14.454H9.625a3.45 3.45 0 0 0-3.45 3.45v11.5a3.45 3.45 0 0 0 3.45 3.45h28.75a3.45 3.45 0 0 0 3.45-3.45v-11.5a3.45 3.45 0 0 0-3.45-3.45m-23.517 4.313a2.59 2.59 0 0 1 2.587 2.587v2.3a2.588 2.588 0 0 1-5.175 0v-2.3a2.59 2.59 0 0 1 2.588-2.587m18.284 0a2.59 2.59 0 0 1 2.588 2.587v2.3a2.588 2.588 0 0 1-5.175 0v-2.3a2.59 2.59 0 0 1 2.587-2.587"/></g></svg>';console.log(`
 %c Post-Abstract-AI 博客文章摘要AI生成工具 %c https://github.com/zhheo/Post-Abstract-AI 
`,"color: #fadfa3; background: #030307; padding:5px 0;","background: #fadfa3; padding:5px 0;");let h=!1;function P(u){function g(t){y();let n=document.querySelector(t);if(!n){let m=0;const f=setInterval(()=>{if(m+=300,n=document.querySelector(t),n)clearInterval(f);else if(m>=2e4){clearInterval(f),console.log("超时未找到元素");return}},300);return}const o=document.createElement("div");o.className="post-TianliGPT";const i=document.createElement("div");i.className="tianliGPT-title",o.appendChild(i);const a=document.createElement("i");a.className="tianliGPT-title-icon",i.appendChild(a),a.innerHTML=p;const r=document.createElement("div");r.className="tianliGPT-title-text",typeof tianliGPT_Title>"u"?r.textContent="AI摘要":r.textContent=tianliGPT_Title,i.appendChild(r);const e=document.createElement("div");e.className="tianliGPT-tag",e.id="tianliGPT-tag",typeof tianliGPT_Name>"u"?e.textContent="TianliGPT":e.textContent=tianliGPT_Name,i.appendChild(e);const s=document.createElement("div");s.className="tianliGPT-explanation",s.innerHTML='生成中...<span class="blinking-cursor"></span>',o.appendChild(s),n.insertBefore(o,n.firstChild)}function y(){var n;const t=document.querySelector(".post-TianliGPT");t&&((n=t.parentElement)==null||n.removeChild(t))}const l={getTitleAndContent:function(){try{const t=document.title,n=document.querySelector(tianliGPT_postSelector);if(!n)return console.warn("TianliGPT：找不到文章容器。请尝试将引入的代码放入到文章容器之后。如果本身没有打算使用摘要功能可以忽略此提示。"),"";const o=n.getElementsByTagName("p"),i=n.querySelectorAll("h1, h2, h3, h4, h5");let a="";for(let c of i)a+=c.innerText+" ";for(let c of o){const d=c.innerText.replace(/https?:\/\/[^\s]+/g,"");a+=d}const r=t+" "+a;let e=1e3;return typeof tianliGPT_wordLimit<"u"&&(e=tianliGPT_wordLimit),r.slice(0,e)}catch(t){return console.error("TianliGPT错误：可能由于一个或多个错误导致没有正常运行，原因出在获取文章容器中的内容失败，或者可能是在文章转换过程中失败。",t),""}},fetchTianliGPT:async function(t){if(typeof aiexcerpt<"u"&&aiexcerpt){const i=aiexcerpt;return l.aiShowAnimation(i),i}if(!tianliGPT_key){const i="没有获取到key，代码可能没有安装正确。如果你需要在tianli_gpt文件引用前定义tianliGPT_key变量。详细请查看文档。";return l.aiShowAnimation(i),i}if(tianliGPT_key==="5Q5mpqRK5DkwT1X9Gi5e"){const i="请购买 key 使用，如果你能看到此条内容，则说明代码安装正确。";return l.aiShowAnimation(i),i}const n="https://summary.tianli0.top/",o=2e4;try{const i=new AbortController,a=setTimeout(()=>i.abort(),o),r=await fetch(n,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:t,key:tianliGPT_key,url:window.location.href,title:document.title}),signal:i.signal});if(clearTimeout(a),r.ok)return{summary:(await r.json()).summary};{let e="";const s=await r.json();if(r.status===514)return e="TianliGPT is only available in mainland China, and is not yet open to overseas users, so stay tuned!",l.aiShowAnimation(e),e;if(r.status===403)switch(s.err_code){case 1:return e='你的网站设置了Referrer-Policy为same-origin，这会导致Tianli无法验证你的请求来源。TianliGPT依赖refer进行来源判断，特别是meta标签的referrer属性需要修改，至少为origin。例如：<meta name="referrer" content="origin">',l.aiShowAnimation(e),e;case 2:return e="你正在使用的tianliGPT_key没有绑定当前网站，请检查当前的密钥是否绑定了当前网站地址。可以到summary.zhheo.com中绑定。",l.aiShowAnimation(e),e;case 3:return e="参数缺失，请检查是否正确配置tianliGPT_key",l.aiShowAnimation(e),e;case 4:throw document.querySelectorAll(".post-TianliGPT").forEach(c=>{c.style.display="none"}),e="Key错误或余额不足，请充值后请求新的文章",new Error("TianliGPT："+e);case 5:return e=s.err_msg,l.aiShowAnimation(e),e;case 6:return e=s.err_msg,l.aiShowAnimation(e),e;case 7:return e=s.err_msg,l.aiShowAnimation(e),e;default:return l.aiShowAnimation("未知错误，请检查API文档"),"未知错误，请检查API文档"}return l.aiShowAnimation("获取文章摘要失败，请稍后再试。"),"获取文章摘要失败，请稍后再试。"}}catch(i){return i.name==="AbortError"?window.location.hostname==="localhost"?(console.warn("警告：请勿在本地主机上测试 API 密钥。"),"获取文章摘要超时。请勿在本地主机上测试 API 密钥。"):(console.error("请求超时"),"获取文章摘要超时。当你出现这个问题时，可能是key或者绑定的域名不正确。也可能是因为文章过长导致的 AI 运算量过大，您可以稍等一下然后刷新页面重试。"):(console.error("请求失败：",i),"获取文章摘要失败，请稍后再试。")}},aiShowAnimation:function(t=""){const n=document.querySelector(".tianliGPT-explanation");if(!n||h)return;if(typeof tianliGPT_typingAnimate<"u"&&!tianliGPT_typingAnimate){n.innerHTML=t;return}h=!0;const o=25,i=6;n.style.display="block",n.innerHTML='生成中...<span class="blinking-cursor"></span>';const a=document.querySelector(".tianliGPT-tag");a==null||a.classList.add("loadingAI");let r=!0,e=0,s=!0,c=performance.now();const d=()=>{if(e<t.length&&r){const T=performance.now(),S=T-c,k=t.slice(e,e+1),_=/[，。！、？,.!?]/.test(k)?o*i:o;if(S>=_)if(n.innerText=t.slice(0,e+1),c=T,e++,e<t.length)n.innerHTML=t.slice(0,e)+'<span class="blinking-cursor"></span>';else{n.innerHTML=t,n.style.display="block",h=!1,m.disconnect();const w=document.querySelector(".tianliGPT-tag");w==null||w.classList.remove("loadingAI")}requestAnimationFrame(d)}},m=new IntersectionObserver(T=>{r=T[0].isIntersecting,r&&s&&setTimeout(()=>{requestAnimationFrame(d)},200)},{threshold:0});let f=document.querySelector(".post-TianliGPT");f&&m.observe(f)}};function G(){if(typeof tianliGPT_postSelector>"u")return;g(tianliGPT_postSelector);const t=l.getTitleAndContent();t&&console.log("TianliGPT本次提交的内容为："+t),l.fetchTianliGPT(t).then(n=>{if(typeof n=="string")return;const o=n.summary;l.aiShowAnimation(o)})}function v(){if(typeof tianliGPT_postURL>"u"){A();return}try{const t=a=>new RegExp("^"+a.split(/\*+/).map(n).join(".*")+"$"),n=a=>a.replace(/[|\\{}()[\]^$+*?.]/g,"\\$&"),o=t(tianliGPT_postURL),i=window.location.href;o.test(i)?x():console.log(`TianliGPT：当前 URL '${i}' 不符合规则 '${tianliGPT_postURL}'，所以我决定不执行摘要功能。`)}catch(t){console.error("TianliGPT：我没有看懂你编写的自定义链接规则，所以我决定不执行摘要功能",t)}}function x(){let n=0;const o=setInterval(()=>{try{A(),clearInterval(o)}catch(i){n>=20&&(clearInterval(o),console.error("TianliGPT：超时失败，停止尝试。",i)),n++}},200)}function A(){if(typeof tianliGPT_blacklist>"u"||!tianliGPT_blacklist){G();return}else fetch(tianliGPT_blacklist).then(t=>t.json()).then(t=>{const n=t.blackurls;let o=window.location.href;n.some(a=>new RegExp("^"+a.replace(/\*/g,".*")+"$").test(o))||G()}).catch(t=>{console.error("请求黑名单失败。Error fetching blacklist:",t),G()})}u?v():document.addEventListener("DOMContentLoaded",function(){v()})}P(!1),document.addEventListener("pjax:complete",function(){P(!0)}),function(u){const g=u.pushState;u.pushState=function(y){return typeof u.onpushstate=="function"&&setTimeout(function(){u.onpushstate({state:y})},100),g.apply(u,arguments)}}(window.history),window.history.onpushstate=function(){P(!0)}});
