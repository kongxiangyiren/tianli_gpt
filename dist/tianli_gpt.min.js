...
(function(f){typeof define=="function"&&define.amd?define(f):f()})(function(){"use strict";const f='<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><g id="机器人" fill="none" fill-rule="evenodd" stroke="none" stroke-width="1"><path id="形状结合" fill="#444" fill-rule="nonzero" d="M34.718 5.036a2.59 2.59 0 0 1 2.127 2.977l-1.05 6.302h2.58A8.625 8.625 0 0 1 47 22.94v11.5a8.625 8.625 0 0 1-8.625 8.625H9.625A8.625 8.625 0 0 1 1 34.44v-11.5a8.625 8.625 0 0 1 8.625-8.625h2.58l-1.05-6.302a2.588 2.588 0 0 1 5.105-.85l1.15 6.9q.021.127.03.252h13.12q.009-.126.03-.253l1.15-6.9a2.59 2.59 0 0 1 2.978-2.126m3.657 14.454H9.625a3.45 3.45 0 0 0-3.45 3.45v11.5a3.45 3.45 0 0 0 3.45 3.45h28.75a3.45 3.45 0 0 0 3.45-3.45v-11.5a3.45 3.45 0 0 0-3.45-3.45m-23.517 4.313a2.59 2.59 0 0 1 2.587 2.587v2.3a2.588 2.588 0 0 1-5.175 0v-2.3a2.59 2.59 0 0 1 2.588-2.587m18.284 0a2.59 2.59 0 0 1 2.588 2.587v2.3a2.588 2.588 0 0 1-5.175 0v-2.3a2.59 2.59 0 0 1 2.587-2.587"/></g></svg>';console.log(`
 %c Post-Abstract-AI 开源博客文章摘要AI生成工具 %c https://github.com/zhheo/Post-Abstract-AI 
`,"color: #fadfa3; background: #030307; padding:5px 0;","background: #fadfa3; padding:5px 0;");function h(i){let c=!1;function a(t){T();const n=document.querySelector(t);if(!n)return;const l=document.createElement("div");l.className="post-TianliGPT";const s=document.createElement("div");s.className="tianliGPT-title",l.appendChild(s);const e=document.createElement("i");e.className="tianliGPT-title-icon",s.appendChild(e),e.innerHTML=f;const o=document.createElement("div");o.className="tianliGPT-title-text",o.textContent="AI摘要",s.appendChild(o);const r=document.createElement("div");r.className="tianliGPT-tag",r.id="tianliGPT-tag",r.textContent="TianliGPT",s.appendChild(r);const u=document.createElement("div");u.className="tianliGPT-explanation",u.innerHTML='生成中...<span class="blinking-cursor"></span>',l.appendChild(u),n.insertBefore(l,n.firstChild)}function T(){var n;const t=document.querySelector(".post-TianliGPT");t&&((n=t.parentElement)==null||n.removeChild(t))}const m={getTitleAndContent:function(){try{const t=document.title,n=document.querySelector(tianliGPT_postSelector);if(!n)return console.warn("TianliGPT：找不到文章容器。请尝试将引入的代码放入到文章容器之后。如果本身没有打算使用摘要功能可以忽略此提示。"),"";const l=n.getElementsByTagName("p"),s=n.querySelectorAll("h1, h2, h3, h4, h5");let e="";for(let p of s)e+=p.innerText+" ";for(let p of l){const g=p.innerText.replace(/https?:\/\/[^\s]+/g,"");e+=g}const o=t+" "+e;let r=1e3;return typeof tianliGPT_wordLimit<"u"&&(r=tianliGPT_wordLimit),o.slice(0,r)}catch(t){return console.error("TianliGPT错误：可能由于一个或多个错误导致没有正常运行，原因出在获取文章容器中的内容失败，或者可能是在文章转换过程中失败。",t),""}},fetchTianliGPT:async function(t){if(!tianliGPT_key)return"没有获取到key，代码可能没有安装正确。如果你需要在tianli_gpt文件引用前定义tianliGPT_key变量。详细请查看文档。";if(tianliGPT_key==="5Q5mpqRK5DkwT1X9Gi5e")return"请购买 key 使用，如果你能看到此条内容，则说明代码安装正确。";const n=window.location.href,l=document.title,s=`https://summary.tianli0.top/?content=${encodeURIComponent(t)}&key=${encodeURIComponent(tianliGPT_key)}&url=${encodeURIComponent(n)}&title=${encodeURIComponent(l)}`;try{const e=new AbortController,o=await fetch(s,{signal:e.signal});if(o.ok){const r=await o.json();return{summary:r.summary,audioId:r.id}}else throw o.status===402&&document.querySelectorAll(".post-TianliGPT").forEach(r=>{r.style.display="none"}),new Error("TianliGPT：余额不足，请充值后请求新的文章")}catch(e){return e.name==="AbortError"?window.location.hostname==="localhost"?(console.warn("警告：请勿在本地主机上测试 API 密钥。"),"获取文章摘要超时。请勿在本地主机上测试 API 密钥。"):(console.error("请求超时"),"获取文章摘要超时。当你出现这个问题时，可能是key或者绑定的域名不正确。也可能是因为文章过长导致的 AI 运算量过大，您可以稍等一下然后刷新页面重试。"):(console.error("请求失败：",e),"获取文章摘要失败，请稍后再试。")}},aiShowAnimation:function(t=""){const n=document.querySelector(".tianliGPT-explanation");if(!n||c)return;if(typeof tianliGPT_typingAnimate<"u"&&!tianliGPT_typingAnimate){n.innerHTML=t;return}c=!0;const l=25,s=6;n.style.display="block",n.innerHTML='生成中...<span class="blinking-cursor"></span>';let e=!0,o=0,r=!0,u=performance.now();const p=()=>{if(o<t.length&&e){const y=performance.now(),L=y-u,k=t.slice(o,o+1),A=/[，。！、？,.!?]/.test(k)?l*s:l;L>=A&&(n.innerText=t.slice(0,o+1),u=y,o++,o<t.length?n.innerHTML=t.slice(0,o)+'<span class="blinking-cursor"></span>':(n.innerHTML=t,n.style.display="block",c=!1,g.disconnect())),requestAnimationFrame(p)}},g=new IntersectionObserver(y=>{e=y[0].isIntersecting,e&&r&&setTimeout(()=>{requestAnimationFrame(p)},200)},{threshold:0});let G=document.querySelector(".post-TianliGPT");G&&g.observe(G)}};function P(){a(tianliGPT_postSelector);const t=m.getTitleAndContent();t&&console.log("TianliGPT本次提交的内容为："+t),m.fetchTianliGPT(t).then(n=>{const l=n.summary,s=n.audioId,e=document.querySelector(".tianliGPT-tag");e&&(e.dataset.audioId=s,m.aiShowAnimation(l))})}function w(){if(typeof tianliGPT_postURL>"u"){P();return}try{const t=e=>new RegExp("^"+e.split(/\*+/).map(n).join(".*")+"$"),n=e=>e.replace(/[|\\{}()[\]^$+*?.]/g,"\\{{input}}"),l=t(tianliGPT_postURL),s=window.location.href;l.test(s)?P():console.log("TianliGPT：因为不符合自定义的链接规则，我决定不执行摘要功能。")}catch(t){console.error("TianliGPT：我没有看懂你编写的自定义链接规则，所以我决定不执行摘要功能",t)}}i?w():document.addEventListener("DOMContentLoaded",function(){w()})}h(!1),document.addEventListener("pjax:complete",function(){h(!0)}),document.addEventListener("click",function(i){const c=i.target;c&&c.classList.contains("tianliGPT-tag")&&v()});let d=null;function v(){if(d&&!d.paused)d.pause();else{if(!d){const a=document.querySelector(".tianliGPT-tag");if(!a)return;const T=a.dataset.audioId;if(!T){console.error("未找到音频 ID");return}const m=`https://summary.tianli0.top/audio?id=${encodeURIComponent(T)}&key=${encodeURIComponent(tianliGPT_key)}`;d=new Audio(m),d.addEventListener("ended",()=>{a.classList.remove("playing")}),d.addEventListener("play",()=>{a.classList.add("playing")}),d.addEventListener("pause",()=>{a.classList.remove("playing")})}setTimeout(()=>{d==null||d.play().catch(a=>{console.error("播放音频失败:",a)})},100)}const i=document.querySelector(".tianliGPT-tag");if(!i)return;i.addEventListener("click",function(){i.classList.toggle("playing")});const c='<?xml version="1.0" standalone="no"?><!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd"><svg t="1692880829044" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2346" data-darkreader-inline-fill="" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20"><path d="M512 1024a512 512 0 1 0 0-1024 512 512 0 1 0 0 1024z m0-704a192 192 0 1 1 0 384 192 192 0 1 1 0-384z" p-id="2347"></path></svg>';i.classList.contains("playing")?(i.innerHTML=c,i.style.animation="",i.style.color="#ff0000",i.style.backgroundColor="transparent",i.style.padding="0"):(i.innerHTML=c,i.style.animation="",i.style.color="#000000",i.style.backgroundColor="transparent",i.style.padding="0")}document.addEventListener("click",function(i){const c=i.target,a=document.querySelector(".tianliGPT-tag");a&&(a.addEventListener("click",function(){a.classList.toggle("playing")}),(c===a||a.contains(c))&&v())})});
