...
(function(m){typeof define=="function"&&define.amd?define(m):m()})(function(){"use strict";const m='<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><g id="机器人" fill="none" fill-rule="evenodd" stroke="none" stroke-width="1"><path id="形状结合" fill="#444" fill-rule="nonzero" d="M34.718 5.036a2.59 2.59 0 0 1 2.127 2.977l-1.05 6.302h2.58A8.625 8.625 0 0 1 47 22.94v11.5a8.625 8.625 0 0 1-8.625 8.625H9.625A8.625 8.625 0 0 1 1 34.44v-11.5a8.625 8.625 0 0 1 8.625-8.625h2.58l-1.05-6.302a2.588 2.588 0 0 1 5.105-.85l1.15 6.9q.021.127.03.252h13.12q.009-.126.03-.253l1.15-6.9a2.59 2.59 0 0 1 2.978-2.126m3.657 14.454H9.625a3.45 3.45 0 0 0-3.45 3.45v11.5a3.45 3.45 0 0 0 3.45 3.45h28.75a3.45 3.45 0 0 0 3.45-3.45v-11.5a3.45 3.45 0 0 0-3.45-3.45m-23.517 4.313a2.59 2.59 0 0 1 2.587 2.587v2.3a2.588 2.588 0 0 1-5.175 0v-2.3a2.59 2.59 0 0 1 2.588-2.587m18.284 0a2.59 2.59 0 0 1 2.588 2.587v2.3a2.588 2.588 0 0 1-5.175 0v-2.3a2.59 2.59 0 0 1 2.587-2.587"/></g></svg>';console.log(`
 %c Post-Abstract-AI 博客文章摘要AI生成工具 %c https://github.com/zhheo/Post-Abstract-AI 
`,"color: #fadfa3; background: #030307; padding:5px 0;","background: #fadfa3; padding:5px 0;");function h(w){let f=!1;function A(t){v();const n=document.querySelector(t);if(!n)return;const r=document.createElement("div");r.className="post-TianliGPT";const s=document.createElement("div");s.className="tianliGPT-title",r.appendChild(s);const a=document.createElement("i");a.className="tianliGPT-title-icon",s.appendChild(a),a.innerHTML=m;const i=document.createElement("div");i.className="tianliGPT-title-text",typeof tianliGPT_Title>"u"?i.textContent="AI摘要":i.textContent=tianliGPT_Title,s.appendChild(i);const o=document.createElement("div");o.className="tianliGPT-tag",o.id="tianliGPT-tag",typeof tianliGPT_Name>"u"?o.textContent="TianliGPT":o.textContent=tianliGPT_Name,s.appendChild(o);const c=document.createElement("div");c.className="tianliGPT-explanation",c.innerHTML='生成中...<span class="blinking-cursor"></span>',r.appendChild(c),n.insertBefore(r,n.firstChild)}function v(){var n;const t=document.querySelector(".post-TianliGPT");t&&((n=t.parentElement)==null||n.removeChild(t))}const l={getTitleAndContent:function(){try{const t=document.title,n=document.querySelector(tianliGPT_postSelector);if(!n)return console.warn("TianliGPT：找不到文章容器。请尝试将引入的代码放入到文章容器之后。如果本身没有打算使用摘要功能可以忽略此提示。"),"";const r=n.getElementsByTagName("p"),s=n.querySelectorAll("h1, h2, h3, h4, h5");let a="";for(let e of s)a+=e.innerText+" ";for(let e of r){const u=e.innerText.replace(/https?:\/\/[^\s]+/g,"");a+=u}const i=t+" "+a;let o=1e3;return typeof tianliGPT_wordLimit<"u"&&(o=tianliGPT_wordLimit),i.slice(0,o)}catch(t){return console.error("TianliGPT错误：可能由于一个或多个错误导致没有正常运行，原因出在获取文章容器中的内容失败，或者可能是在文章转换过程中失败。",t),""}},fetchTianliGPT:async function(t){if(!tianliGPT_key){const i="没有获取到key，代码可能没有安装正确。如果你需要在tianli_gpt文件引用前定义tianliGPT_key变量。详细请查看文档。";return l.aiShowAnimation(i),i}if(tianliGPT_key==="5Q5mpqRK5DkwT1X9Gi5e"){const i="请购买 key 使用，如果你能看到此条内容，则说明代码安装正确。";return l.aiShowAnimation(i),i}const n=window.location.href,r=document.title,s=`https://summary.tianli0.top/?content=${encodeURIComponent(t)}&key=${encodeURIComponent(tianliGPT_key)}&url=${encodeURIComponent(n)}&title=${encodeURIComponent(r)}`,a=2e4;try{const i=new AbortController,o=setTimeout(()=>i.abort(),a),c=await fetch(s,{signal:i.signal});if(clearTimeout(o),c.ok)return{summary:(await c.json()).summary};{let e="";const u=await c.json();if(c.status===514)return e="TianliGPT is only available in mainland China, and is not yet open to overseas users, so stay tuned!",l.aiShowAnimation(e),e;if(c.status===403)switch(u.err_code){case 1:return e='你的网站设置了Referrer-Policy为same-origin，这会导致Tianli无法验证你的请求来源。TianliGPT依赖refer进行来源判断，特别是meta标签的referrer属性需要修改，至少为origin。例如：<meta name="referrer" content="origin">',l.aiShowAnimation(e),e;case 2:return e="你正在使用的tianliGPT_key已经被其他网站绑定或不存在，请检查当前网站地址是否在summary.zhheo.com中已绑定。",l.aiShowAnimation(e),e;case 3:return e="参数缺失，请检查是否正确配置tianliGPT_key",l.aiShowAnimation(e),e;case 4:throw document.querySelectorAll(".post-TianliGPT").forEach(d=>{d.style.display="none"}),e="Key错误或余额不足，请充值后请求新的文章",new Error("TianliGPT："+e);case 5:return e=u.err_msg,l.aiShowAnimation(e),e;case 6:return e=u.err_msg,l.aiShowAnimation(e),e;case 7:return e=u.err_msg,l.aiShowAnimation(e),e;default:return l.aiShowAnimation("未知错误，请检查API文档"),"未知错误，请检查API文档"}return"获取文章摘要失败，请稍后再试。"}}catch(i){return i.name==="AbortError"?window.location.hostname==="localhost"?(console.warn("警告：请勿在本地主机上测试 API 密钥。"),"获取文章摘要超时。请勿在本地主机上测试 API 密钥。"):(console.error("请求超时"),"获取文章摘要超时。当你出现这个问题时，可能是key或者绑定的域名不正确。也可能是因为文章过长导致的 AI 运算量过大，您可以稍等一下然后刷新页面重试。"):(console.error("请求失败：",i),"获取文章摘要失败，请稍后再试。")}},aiShowAnimation:function(t=""){const n=document.querySelector(".tianliGPT-explanation");if(!n||f)return;if(typeof tianliGPT_typingAnimate<"u"&&!tianliGPT_typingAnimate){n.innerHTML=t;return}f=!0;const r=25,s=6;n.style.display="block",n.innerHTML='生成中...<span class="blinking-cursor"></span>';const a=document.querySelector(".tianliGPT-tag");a==null||a.classList.add("loadingAI");let i=!0,o=0,c=!0,e=performance.now();const u=()=>{if(o<t.length&&i){const T=performance.now(),G=T-e,I=t.slice(o,o+1),_=/[，。！、？,.!?]/.test(I)?r*s:r;if(G>=_)if(n.innerText=t.slice(0,o+1),e=T,o++,o<t.length)n.innerHTML=t.slice(0,o)+'<span class="blinking-cursor"></span>';else{n.innerHTML=t,n.style.display="block",f=!1,d.disconnect();const p=document.querySelector(".tianliGPT-tag");p==null||p.classList.remove("loadingAI")}requestAnimationFrame(u)}},d=new IntersectionObserver(T=>{i=T[0].isIntersecting,i&&c&&setTimeout(()=>{requestAnimationFrame(u)},200)},{threshold:0});let y=document.querySelector(".post-TianliGPT");y&&d.observe(y)}};function P(){A(tianliGPT_postSelector);const t=l.getTitleAndContent();t&&console.log("TianliGPT本次提交的内容为："+t),l.fetchTianliGPT(t).then(n=>{if(typeof n=="string")return;const r=n==null?void 0:n.summary;l.aiShowAnimation(r)})}function g(){if(typeof tianliGPT_postURL>"u"){P();return}try{const t=a=>new RegExp("^"+a.split(/\*+/).map(n).join(".*")+"$"),n=a=>a.replace(/[|\\{}()[\]^$+*?.]/g,"\\{{input}}"),r=t(tianliGPT_postURL),s=window.location.href;r.test(s)?P():console.log("TianliGPT：因为不符合自定义的链接规则，我决定不执行摘要功能。")}catch(t){console.error("TianliGPT：我没有看懂你编写的自定义链接规则，所以我决定不执行摘要功能",t)}}w?g():document.addEventListener("DOMContentLoaded",function(){g()})}h(!1),document.addEventListener("pjax:complete",function(){h(!0)})});
