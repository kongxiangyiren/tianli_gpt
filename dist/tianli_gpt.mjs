...
function G(u) {
  function p(t) {
    ...
    if (!n) {
      let d = 0;
      const f = setInterval(() => {
        if (d += 300, n = document.querySelector(t), n)
          clearInterval(f);
        else if (d >= 2e4) {
          clearInterval(f), console.log("超时未找到元素");
          return;
        }
      }, 300);
      return;
    ...
    a.className = "tianliGPT-title-icon", i.appendChild(a), a.innerHTML = _;
  ...
  const l = {
    ...
    getTitleAndContent: function() {
      try {
        ...
        for (let c of i)
          a += c.innerText + " ";
        for (let c of o) {
          const m = c.innerText.replace(/https?:\/\/[^\s]+/g, "");
          a += m;
        }
        const r = t + " " + a;
        let e = 1e3;
          ...
          };
        {
          ...
          if (r.status === 403)
            switch (s.err_code) {
              ...
              case 2:
                return e = "你正在使用的tianliGPT_key没有绑定当前网站，请检查当前的密钥是否绑定了当前网站地址。可以到summary.zhheo.com中绑定。", l.aiShowAnimation(e), e;
    ...
    aiShowAnimation: function(t = "") {
      ...
      const m = () => {
        if (e < t.length && r) {
          const T = performance.now(), A = T - c, x = t.slice(e, e + 1), k = /[，。！、？,.!?]/.test(x) ? o * i : o;
          if (A >= k)
            if (n.innerText = t.slice(0, e + 1), c = T, e++, e < t.length)
              n.innerHTML = t.slice(0, e) + '<span class="blinking-cursor"></span>';
            else {
              n.innerHTML = t, n.style.display = "block", y = !1, d.disconnect();
        ...
        (T) => {
          r = T[0].isIntersecting, r && s && setTimeout(() => {
            requestAnimationFrame(m);
          }, 200);
        },
        { threshold: 0 }
      );
      let f = document.querySelector(".post-TianliGPT");
      f && d.observe(f);
    }
  };
  function P() {
    if (typeof tianliGPT_postSelector > "u")
      return;
    p(tianliGPT_postSelector);
  ...
  function v() {
    if (typeof tianliGPT_blacklist > "u" || !tianliGPT_blacklist) {
      P();
      return;
    } else
      fetch(tianliGPT_blacklist).then((t) => t.json()).then((t) => {
        ...
        n.some((a) => new RegExp("^" + a.replace(/\*/g, ".*") + "$").test(o)) || P();
      }).catch((t) => {
        console.error("请求黑名单失败。Error fetching blacklist:", t), P();
      });
  }
  u ? w() : document.addEventListener("DOMContentLoaded", function() {
    w();
  });
}
G(!1);
