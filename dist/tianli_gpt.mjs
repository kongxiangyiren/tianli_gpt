...
function P(p) {
  ...
  function r(n) {
    ...
    e.className = "tianliGPT-title-icon", a.appendChild(e), e.innerHTML = A;
    const i = document.createElement("div");
    i.className = "tianliGPT-title-text", i.textContent = "AI摘要", a.appendChild(i);
    const l = document.createElement("div");
    l.className = "tianliGPT-tag", l.id = "tianliGPT-tag", l.textContent = "TianliGPT", a.appendChild(l);
    const c = document.createElement("div");
    c.className = "tianliGPT-explanation", c.innerHTML = '生成中...<span class="blinking-cursor"></span>', o.appendChild(c), t.insertBefore(o, t.firstChild);
  }
  function G() {
    var t;
    const n = document.querySelector(".post-TianliGPT");
    n && ((t = n.parentElement) == null || t.removeChild(n));
  }
  const T = {
    //读取文章中的所有文本
    getTitleAndContent: function() {
      try {
        const n = document.title, t = document.querySelector(tianliGPT_postSelector);
        if (!t)
          return console.warn(
            "TianliGPT：找不到文章容器。请尝试将引入的代码放入到文章容器之后。如果本身没有打算使用摘要功能可以忽略此提示。"
          ), "";
        const o = t.getElementsByTagName("p"), a = t.querySelectorAll("h1, h2, h3, h4, h5");
        let e = "";
        for (let u of a)
          e += u.innerText + " ";
        for (let u of o) {
          const d = u.innerText.replace(/https?:\/\/[^\s]+/g, "");
          e += d;
        }
        const i = n + " " + e;
        let l = 1e3;
        return typeof tianliGPT_wordLimit < "u" && (l = tianliGPT_wordLimit), i.slice(0, l);
      } catch (n) {
    ...
    fetchTianliGPT: async function(n) {
      if (!tianliGPT_key)
      ...
      const t = window.location.href, o = document.title, a = `https://summary.tianli0.top/?content=${encodeURIComponent(
        n
      )}&key=${encodeURIComponent(tianliGPT_key)}&url=${encodeURIComponent(
        t
      )}&title=${encodeURIComponent(o)}`;
      try {
        const e = new AbortController(), i = await fetch(a, { signal: e.signal });
        if (i.ok) {
          ...
          throw i.status === 402 && document.querySelectorAll(".post-TianliGPT").forEach((l) => {
    ...
    aiShowAnimation: function(n = "") {
      const t = document.querySelector(".tianliGPT-explanation");
      if (!t || s)
        return;
      ...
      s = !0;
      const o = 25, a = 6;
      t.style.display = "block", t.innerHTML = '生成中...<span class="blinking-cursor"></span>';
      let e = !0, i = 0, l = !0, c = performance.now();
      const u = () => {
        if (i < n.length && e) {
          const m = performance.now(), y = m - c, v = n.slice(i, i + 1), w = /[，。！、？,.!?]/.test(v) ? o * a : o;
          y >= w && (t.innerText = n.slice(0, i + 1), c = m, i++, i < n.length ? t.innerHTML = n.slice(0, i) + '<span class="blinking-cursor"></span>' : (t.innerHTML = n, t.style.display = "block", s = !1, d.disconnect())), requestAnimationFrame(u);
        }
      }, d = new IntersectionObserver(
        (m) => {
          e = m[0].isIntersecting, e && l && setTimeout(() => {
            requestAnimationFrame(u);
          }, 200);
        },
        { threshold: 0 }
      );
      let h = document.querySelector(".post-TianliGPT");
      h && d.observe(h);
    }
  };
  function f() {
    r(tianliGPT_postSelector);
    const n = T.getTitleAndContent();
    n && console.log("TianliGPT本次提交的内容为：" + n), T.fetchTianliGPT(n).then((t) => {
      if (typeof t == "string") {
      ...
      const o = t.summary, a = t.audioId, e = document.querySelector(".tianliGPT-tag");
      e && (e.dataset.audioId = a, T.aiShowAnimation(o));
    });
  }
  function g() {
    ...
    try {
      const n = (e) => new RegExp("^" + e.split(/\*+/).map(t).join(".*") + "$"), t = (e) => e.replace(/[|\\{}()[\]^$+*?.]/g, "\\{{input}}"), o = n(tianliGPT_postURL), a = window.location.href;
      o.test(a) ? f() : console.log("TianliGPT：因为不符合自定义的链接规则，我决定不执行摘要功能。");
    } catch (n) {
      console.error("TianliGPT：我没有看懂你编写的自定义链接规则，所以我决定不执行摘要功能", n);
    }
  }
  p ? g() : document.addEventListener("DOMContentLoaded", function() {
    g();
  });
}
P(!1);
...
document.addEventListener("click", function(p) {
  const s = p.target, r = document.querySelector(".tianliGPT-tag");
